...
\end{tabular}
\end{frame}
\begin{frame}{Statystyki opisowe zmiennych ilościowych}\slidetable{../tables/01_descriptive_statistics.tex}\end{frame}
\begin{frame}{Zmiennie binarne i zbinaryzowane}\slidetable{../tables/01b_binary_variables.tex}\end{frame}
\begin{frame}{Struktura zdarzeń i cenzurowania}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{.42\textwidth}\slidetable{../tables/02_censoring_structure.tex}\end{column}
\begin{column}{.56\textwidth}\centering\slidefig{02_censoring_structure.pdf}\end{column}
\end{columns}
...
\begin{frame}{Tablica aktuarialna: wyniki}\slidetable{../tables/04_actuarial_life_table.tex}\end{frame}
\begin{frame}{Tablica aktuarialna: wykres}\centering\widefig{05_actuarial_life_table.pdf}\end{frame}
...
\centering
\widefig{04b_smoothed_hazard_bw50.pdf}\\[-.3em]
...
\end{frame}
\begin{frame}{Kaplan-Meier dla podgrup I}\centering\widefig{06_km_subgroup_1.pdf}\end{frame}
\begin{frame}{Kaplan-Meier dla podgrup II}\centering\widefig{06_km_subgroup_2.pdf}\end{frame}
\begin{frame}{Kaplan-Meier dla podgrup III}\centering\widefig{06_km_subgroup_3.pdf}\end{frame}
\begin{frame}{Kaplan-Meier dla podgrup IV}\centering\widefig{06_km_subgroup_4.pdf}\end{frame}
...
\end{frame}
\begin{frame}{Wyniki testów nieparametrycznych}\slidetable{../tables/05_nonparametric_tests.tex}\end{frame}
...
\end{frame}
\begin{frame}{Porównanie modeli parametrycznych}\slidetable{../tables/07_parametric_model_comparison.tex}\end{frame}
...
\end{itemize}
\end{frame}
\begin{frame}{Parametry pełnego modelu Weibull AFT}\slidetable{../tables/08_best_aft_parameters.tex}\end{frame}
\begin{frame}{Model Weibull AFT zredukowany}\slidetable{../tables/08b_reduced_aft_parameters.tex}\end{frame}
\begin{frame}{Model pełny i zredukowany}\slidetable{../tables/08c_full_vs_reduced_aft.tex}\end{frame}
\begin{frame}{Profile przeżycia w modelu AFT}\centering\widefig{08c_weibull_survival_profile.pdf}\\[-.3em]{\scriptsize Profil wysokiego ryzyka charakteryzuje się szybszym spadkiem funkcji przeżycia w modelu Weibull AFT zredukowanym.}\end{frame}
\begin{frame}{Hazard w modelu AFT}\centering\widefig{08e_weibull_hazard_profile.pdf}\\[-.3em]{\scriptsize Profile ryzyka różnią się poziomem hazardu, przy podobnym kształcie przebiegu wynikającym ze wspólnego parametru rozkładu Weibulla.}\end{frame}
...
\end{frame}
\begin{frame}{Posterior modelu bayesowskiego}\slidetable{../tables/08d_bayesian_posterior.tex}\end{frame}
\begin{frame}{Traceploty MCMC}\centering\widefig{08d_bayesian_traceplots_refreshed.pdf}\\[-.3em]{\scriptsize Średnia akceptacja MCMC wyniosła 0{,}5339, czyli przyjęto około 53{,}4\% proponowanych przejść algorytmu Metropolisa-Hastingsa. Taki poziom oznacza, że łańcuch eksploruje posterior bez nadmiernego odrzucania propozycji.}\end{frame}
...
\end{frame}
\begin{frame}{Wyniki modelu Coxa}\slidetable{../tables/09_cox_model.tex}\end{frame}
\begin{frame}{Współczynniki modelu Coxa}\centering\widefig{09_cox_coefficients.pdf}\\[-.3em]{\scriptsize Wartości dodatnie oznaczają wzrost hazardu, a ujemne jego spadek; poziome odcinki oznaczają przedziały ufności.}\end{frame}
...
\end{frame}
\begin{frame}{Test proporcjonalności hazardów}\slidetable{../tables/10_cox_ph_test.tex}\vspace{-0.4em}{\scriptsize Test wskazuje na naruszenie założenia PH dla frakcji wyrzutowej na poziomie istotności 5\%.}\end{frame}
\begin{frame}{Reszty Schoenfelda I}\centering\widefig{10_schoenfeld_residuals_1.pdf}\end{frame}
\begin{frame}{Reszty Schoenfelda II}\centering\widefig{10_schoenfeld_residuals_2.pdf}\end{frame}
\begin{frame}{Reszty Schoenfelda III}\centering\widefig{10_schoenfeld_residuals_3.pdf}\end{frame}
\begin{frame}{Reszty Schoenfelda IV}\centering\widefig{10_schoenfeld_residuals_4.pdf}\end{frame}
\begin{frame}{Reszty Schoenfelda V}\centering\widefig{10_schoenfeld_residuals_5.pdf}\end{frame}
\begin{frame}{Reszty Schoenfelda z prostą}\centering\widefig{schoenfeld_residuals_v2_1.png}\end{frame}
\begin{frame}{Reszty Schoenfelda z prostą}\centering\widefig{schoenfeld_residuals_v2_2.png}\end{frame}
\begin{frame}{Wyniki modelu Coxa z interakcją czasową}\slidetable{../tables/10c_cox_time_interaction_results.tex}\end{frame}

\begin{frame}{Interpretacja interakcji frakcji wyrzutowej z czasem}
\small

\[
h(t\mid X)=h_0(t)\exp\left(
\ldots+\beta_{\mathrm{EF}}\mathrm{EF}
+\beta_{\mathrm{EF},t}\mathrm{EF}\cdot g(t)
\right),
\qquad g(t)=\log(1+t).
\]
...
\beta_{\mathrm{EF}}
+
\beta_{\mathrm{EF},t}g(t),
\qquad
HR_{\mathrm{EF}}(t)
=
\exp\left(\beta_{\mathrm{EF}}+\beta_{\mathrm{EF},t}g(t)\right).
\]

\begin{itemize}
    \item W modelu start--stop oszacowano:
    \[
    \beta_{\mathrm{EF}}=-0.0130,
    \qquad
    \beta_{\mathrm{EF},t}=-0.0023.
    \]

    \item Dla początku obserwacji, np. \(t=1\):
    \[
    HR_{\mathrm{EF}}(1)
    =
    \exp(-0.0130-0.0023\log 2)
    \approx 0.985.
    \]

    \item Po około miesiącu, dla \(t=30\):
    \[
    HR_{\mathrm{EF}}(30)
    =
    \exp(-0.0130-0.0023\log 31)
    \approx 0.979.
    \]

    \item W późniejszym okresie, dla \(t=100\):
    \[
    HR_{\mathrm{EF}}(100)
    =
    \exp(-0.0130-0.0023\log 101)
    \approx 0.976.
    \]
...
\begin{itemize}
    \item Frakcja wyrzutowa ma efekt ochronny: wzrost o 1 punkt procentowy wiąże się z niższym hazardem zgonu.
    \item Ujemny składnik czasowy sugeruje lekkie wzmacnianie efektu ochronnego w czasie, ale jego istotność jest graniczna \((p\approx 0.084)\).
    \end{itemize}

\end{frame}

\begin{frame}{Model Coxa z interakcją czasową}\slidetable{../tables/10b_cox_time_interaction.tex}\end{frame}
\begin{frame}{Predykcja funkcji przeżycia z modelu Coxa}\centering\widefig{11_cox_survival_three_risk_profiles.pdf}\\[-.3em]{\scriptsize Trzy klasy ryzyka pokazują praktyczną interpretację modelu Coxa: wraz z pogarszaniem profilu klinicznego przewidywana funkcja przeżycia opada coraz szybciej.}\end{frame}
\begin{frame}{Predykcja funkcji hazardu z modelu Coxa}\centering\widefig{11b_cox_hazard_three_risk_profiles.pdf}\\[-.3em]{\scriptsize Funkcja hazardu pokazuje bieżącą intensywność zdarzenia dla profili ryzyka. W modelu Coxa profile różnią się poziomem hazardu przez czynnik $\exp(\beta^T X)$, przy wspólnym hazardzie bazowym.}\end{frame}
\begin{frame}{Predykcja skumulowanego hazardu z modelu Coxa}\centering\widefig{12_additional_cox_prediction_output.png}\\\end{frame}
\begin{frame}{Porównanie klas modeli}\slidetable{../tables/12_model_class_comparison.tex}\end{frame}
